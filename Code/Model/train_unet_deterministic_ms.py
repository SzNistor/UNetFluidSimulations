"""
train_unet_deterministic_ms.py

Train a UNet with deterministic simulations. The flags and the smoke's source are included as an extra channel.
...
# Pair input and output according to step size
...
class DeepFlowDataGen(keras.utils.Sequence):
  ...
  def __data_generation(self, batch_ids):
    ...
    Y = np.empty((self.batch_size, *self.dim, 4))

    # Generate data
    for i, ID in enumerate(batch_ids):
      # Get the simulation number and laod the first id's density as a constant
      first_id = ID[:-10] + "000000.npz"
      x_first = np.load(first_id)["arr_0"]
      x_first = self.remap_obstacles(x_first)
      x_first = self.normalize(x_first)
      ...
      # Prepend the original flags and smoke source
      X[i,] = np.dstack([x_first[:, :, 0:2], x[:, :, 1:]])
      Y[i,] = y[:, :, 1:]
...
def build_unet_model():
  ...
  # outputs
  outputs = layers.Conv2D(4, 1, padding="same", activation = "linear")(u9)

...
scene = sceneModule.instantiate_scene(vorticity=float(args.vorticity_level), resolution=Vec3(args.resolution_x, args.resolution_y, args.resolution_z), boundary=0, dimension=args.dimension, timestep=1, gravity=Vec3(0,-0.003,0), merge_ghost_fluid=True, show_gui=args.gui, pause_on_start=args.pause, **unknown_args)
...
def on_sim_step(scene, t):
    ...
    for grid_name in grids:
        ...
        # Save the current step as the target for the previous step
        if step_file_num-args.skip_steps >= 0:
            # The current scene is the prediction of the scene from before
            label_output_name = "label_{:06d}".format(step_file_num - args.skip_steps)
            grid_io.write_grid(grid, output_path + "/" + grid_name + "_" + label_output_name)
...
for scene_num in range(args.num_scenes):
    ...
    # ADDED BY Szilard Nistor    
    # Flags + Density + Pressure + Vel X + Vel Y
    info("Combining Data.")
    combine_npz_data(path=output_path, params=["flags", "density", "pressure", "vel"])
    # Create a single .npz file for each scene
    info("Cleaning up...")
    files = glob.glob(output_path + "/*.npz")
